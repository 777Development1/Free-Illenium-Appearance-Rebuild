...
CreateThread(function()
    if Config.UseTarget then
        if not Target.AddTargetEntity then
            print("^1[illenium-appearance] Config.UseTarget is enabled, but neither ox_target nor qb-target could be detected on this server (client/target/ox.lua and client/target/qb.lua both found their resource missing). Store/clothing-room/outfit-room targets and the drop-clothing pickup feature are disabled this session. Make sure ox_target (or qb-target) is installed, started, and appears in your server.cfg, ideally before illenium-appearance.^7")
            return
        end
        SetupTargets()
    end
