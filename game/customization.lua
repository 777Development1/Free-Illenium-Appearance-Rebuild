...
function client.zoomCamera(delta)
    if cameraHandle then
        ...
        SetCamFov(cameraHandle, currentFov)
    end
end
local function setCamera(key)
    if not isCameraInterpolating then
        ...
        currentFov = 49.0 -- switching camera presets starts from a fresh zoom level
...
-- Lightweight rotation for continuous input (mouse-drag). pedTurn above queues a
-- walking task-sequence and flips the camera-reverse flag every call, which is fine
-- for a single 10-180 degree button press but breaks down completely when called
-- dozens of times per second from mousemove: each tiny delta interrupts the previous
-- walk task and re-flips the camera flag, so the ped never visually turns. This just
-- sets heading directly, no tasks, no camera side effects, safe to spam every frame.
local function rotatePedByDelta(ped, delta)
    SetEntityHeading(ped, (GetEntityHeading(ped) + delta) % 360.0)
end
client.rotatePedByDelta = rotatePedByDelta
