...
-- Enables chat commands to individually toggle clothing pieces on/off (e.g. /hat, /shirt, /shoes).
-- Running the same command again puts the item back on exactly as it was.
Config.EnableClothingToggleCommands = true
Config.ClothingToggleCommands = {
    -- Ped components (SetPedComponentVariation slots)
    { command = "mask",       component = 1 },
    { command = "arms",       component = 15 },
    { command = "undershirt", component = 15 },
    { command = "pants",      component = 21 },
    { command = "bag",        component = 5 },
    { command = "shirt",      component = 15 }, -- torso2: the visible top/jacket layer
    { command = "jacket",     component = 15 }, -- alias for shirt, same GTA component slot
    { command = "vest",       component = 9 },
    { command = "chain",      component = 7 },
    { command = "decals",     component = 10 },
    { command = "shoes",      component = 5 },

    -- Props (SetPedPropIndex slots)
    { command = "hat",        prop = 0 },
    { command = "glasses",    prop = 1 },
    { command = "ears",       prop = 2 },
    { command = "watch",      prop = 6 },
    { command = "bracelet",   prop = 7 },
}

-- When hit by another player, there's a chance any worn hat/glasses fall to the ground as a
-- pickup-able object. If Config.UseTarget = true (with ox_target or qb-target installed),
-- pickup is offered via target. Otherwise, walk up to it and press E.
Config.DropClothingOnHit = {
    enabled = true,
    pickupDistance = 1.5,
    despawnAfter = 5 * 60000, -- ms before an unclaimed drop disappears on its own; 0 = never despawn
    hat = {
        chance = 35, -- % chance rolled on each hit from another player, only if a hat is worn
        dropModel = `prop_cs_clothes_box`, -- generic placeholder prop; swap for an exact hat mesh if your server has one
        label = "Pick up hat"
    },
    glasses = {
        chance = 35,
        dropModel = `prop_cs_clothes_box`,
        label = "Pick up glasses"
    }
}
